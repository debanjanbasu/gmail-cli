...
//!
//! Run with:
//! `LIVE_E2E=1 cargo test --test live_e2e -- --ignored --nocapture`
//!
//! Credentials: config via `ConfigLoader` (honours `GRR_CONFIG_PATH`); the
//! live test reads `GRR_TOKEN_PATH` or `~/.grr/token.json` and injects it.
//! Normal CLI login stores credentials in the OS keyring, with an automatic
//! cache-directory fallback.

use std::path::PathBuf;
use std::time::{SystemTime, UNIX_EPOCH};

use grr_cli::core::http::TransportInfo;
use grr_cli::core::{ConfigLoader, GoogleAuth, TokenStorage};
use grr_cli::gmail::client::mime_message_stream;
use grr_cli::gmail::models::{Message, MessageBody, MessagePayload, MessageRef};
