...
async fn write_then_read_round_trips() {
    ...
    grr_core::fs_io::write_file(&path, payload.clone())
        .await
        .unwrap();
    let read_back = grr_core::fs_io::read_file(&path).await.unwrap();
...
async fn write_creates_parent_dirs_and_empty_payload_works() {
    ...
    grr_core::fs_io::write_file(&path, Bytes::new())
        .await
        .unwrap();
    assert_eq!(
        grr_core::fs_io::read_file(&path).await.unwrap(),
        Bytes::new()
    );
}
...
async fn io_uring_backend_round_trips_when_available() {
    if !grr_core::runtime::has_io_uring() {
        return;
    }
    let dir = tempfile::tempdir().unwrap();
    ...
    grr_core::fs_io::write_file(&path, payload.clone())
        .await
        .unwrap();
    assert_eq!(grr_core::fs_io::read_file(&path).await.unwrap(), payload);
}
