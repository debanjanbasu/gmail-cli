...
// Zero-config: tokio sizes OS threads to cores; these bound only the
// network side and adapt via server flow control + Retry-After.

/// Hard ceiling on in-flight HTTP requests. Gmail's per-user rate limits
...
impl GmailClientBuilder {
    ...
    pub async fn build(self) -> Result<GmailClient> {
        ...
        let mut http_client = self.http_client.unwrap_or_else(build_http_client);
        ...
        GmailClient::new(auth, http_client, base_url, upload_base_url, info).await
    }
}
...
impl GmailClient {
    ...
    pub async fn new(
        ...
        // Not a thread pool: tokio schedules unbounded green tasks onto
        // OS threads sized to the core count automatically. This valve
        // bounds only in-flight HTTP requests, staying under Gmail's
        // per-user rate limits so bursts don't self-Deny-of-Service into
        // 429s. HTTP/2/3 server flow control multiplexes across pooled
        // connections; more than this would just burn rate quota.
        let semaphore = Arc::new(Semaphore::new(MAX_INFLIGHT_REQUESTS));

        info!(
            "GmailClient initialized: http3={}, io_uring={}, max_inflight_requests={}",
            runtime_features.http3, runtime_features.io_uring, MAX_INFLIGHT_REQUESTS
        );
