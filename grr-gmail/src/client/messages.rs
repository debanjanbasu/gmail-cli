...
impl super::GmailClient {
    ...
    pub async fn get_message_raw_bytes(&self, message_id: &str) -> Result<Bytes> {
        ...
        let data = json
            .get("raw")
            .and_then(|v| v.as_str())
            .ok_or_else(|| GrrError::NotFound("Raw message not available".into()))?;
        ...
        let written = base64::engine::general_purpose::URL_SAFE_NO_PAD
            ...
            .map_err(|e| GrrError::Internal(format!("raw message base64 decode: {e}")))?;
    ...
    pub async fn download_attachment_to(
        ...
        grr_core::fs_io::write_file(path, bytes).await?;
        Ok(len)
    }
