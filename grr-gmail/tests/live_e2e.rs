...
//!
//! [`live_full_pipeline`] exercises the full data plane â€” search, attachment
//! download, streaming import, trash â€” and records the transport negotiated
...
//!
//! Run with:
//! `LIVE_E2E=1 cargo test -p grr-core --test live_e2e -- --ignored --nocapture`
//!
//! Credentials: config via `ConfigLoader` (honours `GRR_CONFIG_PATH`),
//! token from `~/.grr/token.json` (honours `GRR_TOKEN_PATH`;
//! note `GoogleAuth::new` reads the cache-dir copy instead). Set
//! `LIVE_E2E_HTTP3 (removed: HTTP/3 is compile-time)=1` to force `enable_http3` for this run so the QUIC probe
...
use grr_core::{ConfigLoader, GoogleAuth, TokenStorage};
use grr_gmail::client::mime_message_stream;
use grr_gmail::models::{Message, MessageBody, MessagePayload, MessageRef};
use grr_gmail::{GmailClient, GmailClientBuilder};

const LIVE_E2E_ENV: &str = "LIVE_E2E";

const TOKEN_PATH_ENV: &str = "GRR_TOKEN_PATH";
...
/// Full live pipeline: search â†’ attachment download â†’ streaming import â†’
...
async fn live_full_pipeline() {
    ...
    // HTTP/3 is compile-time now (the `http3` feature, on in the CLI);
    // no runtime knob exists to flip.
    let config = ConfigLoader::load().await.expect("load real config");
    ...
    let subject = format!("grr live-e2e {}", timestamp_millis());
