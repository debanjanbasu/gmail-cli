...
async fn write_then_read_round_trips() {
    ...
    gmail_core::fs_io::write_file(&path, payload.clone())
        .await
        .unwrap();
...
async fn write_creates_parent_dirs_and_empty_payload_works() {
    ...
    gmail_core::fs_io::write_file(&path, Bytes::new())
        .await
        .unwrap();
    assert_eq!(
        gmail_core::fs_io::read_file(&path).await.unwrap(),
        Bytes::new()
    );
}
...
async fn io_uring_backend_round_trips_when_available() {
    if !gmail_core::runtime::has_io_uring() {
        return;
    }
    let dir = tempfile::tempdir().unwrap();
