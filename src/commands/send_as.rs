...
use crate::gmail::{CreateSendAsOptions, GmailClient, UpdateSendAsOptions};
...
pub enum SendAsCommands {
    ...
    Delete(DeleteArgs),
    #[command(about = "Verify a send-as alias")]
    Verify(VerifyArgs),
}
...
pub async fn handle_send_as_cmd(client: &GmailClient, cmd: SendAsCommands) -> Result<()> {
    match cmd {
        ...
        SendAsCommands::Verify(args) => {
            let send_as = client.verify_send_as(&args.send_as_email).await?;
            print_output(&send_as, args.format)?;
        }
    }
    Ok(())
