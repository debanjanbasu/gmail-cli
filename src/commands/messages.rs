...
use crate::gmail::{GmailClient, extract_body};
...
pub struct GetArgs {
    ...
    /// Gmail payload format for this message: full, metadata, minimal, raw
    ...
    /// Output format for the printed result (json, jsonl, table, pretty)
    #[arg(short, long, value_enum, default_value = "json")]
    pub format: OutputFormat,
}
...
fn body_json(msg: &crate::gmail::Message, max_length: usize) -> serde_json::Value {
...
pub async fn handle_message_cmd(client: &GmailClient, cmd: MessageCommands) -> Result<()> {
    match cmd {
        ...
        MessageCommands::Get(args) => {
            ...
            if matches!(args.message_format, MessageFormat::Raw) {
                let raw = client.get_message_raw_bytes(&args.message_id).await?;
                print_output(&String::from_utf8_lossy(&raw), args.format)?;
            } else if args.body {
        ...
        MessageCommands::Attachment(args) => {
            if let Some(path) = args.output {
                ...
                print_output(&encoded, args.format)?;
            }
        }
    }
    Ok(())
