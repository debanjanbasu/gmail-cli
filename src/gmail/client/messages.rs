...
/// Decode a base64url (unpadded) string field from a JSON response body
/// into `Bytes` with a single output allocation. `missing` is the
/// `NotFound` message when the field is absent; `context` prefixes the
/// decode-failure message.
async fn decode_base64url_field(
    ...
    let data = json
        .get(field)
        .and_then(|v| v.as_str())
        .ok_or_else(|| GrrError::NotFound(missing.to_string()))?;
    ...
    let written = base64::engine::general_purpose::URL_SAFE_NO_PAD
        ...
        .map_err(|e| GrrError::Internal(format!("{context} base64 decode: {e}")))?;
...
impl super::GmailClient {
    ...
    pub async fn get_message_raw_bytes(&self, message_id: &str) -> Result<Bytes> {
        ...
        decode_base64url_field(response, "raw", "Raw message not available", "raw message").await
    }
    ...
    pub async fn get_attachment_bytes(
        ...
        let data = json
            .get("data")
            .and_then(|v| v.as_str())
            .ok_or_else(|| GrrError::NotFound("Attachment data missing".into()))?;

        let estimated = data.len() * 3 / 4 + 3;
        let mut buf = bytes::BytesMut::zeroed(estimated);
        let written = base64::engine::general_purpose::URL_SAFE_NO_PAD
            .decode_slice(data, &mut buf)
            .map_err(|e| GrrError::Internal(format!("attachment base64 decode: {e}")))?;
        buf.truncate(written);
        Ok(buf.freeze())
    }
    ...
    pub async fn download_attachment_to(
        ...
        crate::core::fs_io::write_file(path, bytes).await?;
        Ok(len)
    }
