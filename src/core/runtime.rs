...
///
/// Only Linux has an io_uring implementation; every other target reports
/// `false` so callers never see an acceleration claim the binary cannot use.
...
mod tests {
    ...
    /// Regression guard: io_uring must be reported only on Linux, where the
    /// implementation is compiled.
    #[test]
    fn io_uring_report_matches_target_gate() {
        assert_eq!(has_io_uring(), cfg!(target_os = "linux"));
        assert_eq!(
            detect_runtime_features().io_uring,
            cfg!(target_os = "linux")
        );
